...
func GoPublishRedis(userID uint64, listZ ...*redis.Z) error {
	...
	pipe.Expire(global.CONTEXT, keyPublish, global.PUBLISH_EXPIRE+global.EXPIRE_TIME_JITTER*time.Duration(rand.Float64()))
...
func GoVideoList(videoList []model.Video) error {
	...
	for _, video := range videoList {
		...
		pipe.Expire(global.CONTEXT, keyVideo, global.VIDEO_EXPIRE+global.EXPIRE_TIME_JITTER*time.Duration(rand.Float64()))
	}
	_, err := pipe.Exec(global.CONTEXT)
	return err
}
...
func GoCommentsOfVideo(commentList []model.Comment, keyCommentsOfVideo string) error {
	...
	pipe.Expire(global.CONTEXT, keyCommentsOfVideo, global.VIDEO_COMMENTS_EXPIRE+global.EXPIRE_TIME_JITTER*time.Duration(rand.Float64()))
	_, err := pipe.Exec(global.CONTEXT)
	return err
}
...
func GetCommentCountOfVideo(videoID uint64) (int, error) {
	...
	values := []interface{}{global.VIDEO_COMMENTS_EXPIRE.Seconds() + global.EXPIRE_TIME_JITTER.Seconds()*rand.Float64()}
...
func SetUserPublishEmpty(userID uint64) error {
	...
	return global.REDIS.Set(global.CONTEXT, keyEmpty, "1", global.EMPTY_EXPIRE+global.EXPIRE_TIME_JITTER*time.Duration(rand.Float64())).Err()
}
