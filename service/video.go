...
func GetVideos(LatestTime time.Time) (videos []dao.Video, rowsAffected int64) {
...
func PublishVideo(video *dao.Video) error {
	//生成增长的 VideoID
...
func SetActive(videoI uint64) error {
	err := global.GVAR_DB.Debug().Model(&dao.Video{}).Where("video_id = ?", videoI).Update("active", true).Error
	if err != nil {
		return err
	}
	return nil
...
func GetPublishedVideos(userID uint64) (videos []dao.Video, err error) {
